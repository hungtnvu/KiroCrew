...
class TestStagingDoesNotWriteToTheLiveDatabase:
    ...
    def test_a_symlinked_data_home_does_not_traceback(
        ...
        which calls `open_dir_pinned` OUTSIDE its own try and resolves only the PARENT
...
class TestACorruptCoreDatabaseFailsInsteadOfPruningAGoodBackup:
    ...
    def test_a_tree_database_that_is_not_sqlite_still_rides_as_bytes(
        ...
        A `.db` found by the tree walk is incidental -- a file the operator happens to keep
        in their workspace -- and refusing the whole snapshot over it would be an outage
        rather than a safeguard. `_setup_fake_kirocrew` leaves a stub `kb.sqlite3` under
        workspace/knowledge that is not a real database; it must ride as bytes.
        """
        archive = snap._build_snapshot(
