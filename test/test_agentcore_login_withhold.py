...
def test_login_rebuild_aborts_when_app_agent_refresh_fails(
    ...
    """A failed replacement must keep originals and abort the login emit."""
    from kiro_crew.agent import rebuild_agent_config
    from kiro_crew.apps import bridges

    kiro_dir = _seed_rebuild_sources(tmp_path, monkeypatch)
    prior = (kiro_dir / "kirocrew.json").read_text(encoding="utf-8")
    ...
    original.write_text(original_body, encoding="utf-8")
    ...
    try:
        ...
        reset_context()
    assert (kiro_dir / "kirocrew.json").read_text(encoding="utf-8") == prior
    assert original.read_text(encoding="utf-8") == original_body
...
def test_stash_propagates_unreadable_existing(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    """A transient stash read must not replace withheld definitions."""
    from kiro_crew.agent import AUTHORED_MCP_SIDECAR, _stash_authored_mcp
    from kiro_crew.config import config_dir
    ...
    prior = sidecar.read_text(encoding="utf-8")
