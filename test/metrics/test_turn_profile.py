...
class TestServedBackendAttribution:
    ...
    def test_label_distinguishes_the_backends_the_config_cannot(self):
        ...
        # An unrecognised object degrades to what the config said unconditionally,
        # so the change can never be worse than what it replaced.
        assert provider_label(object()) == PROVIDER_LABEL_DEFAULT

    def test_the_emit_site_reads_the_label_not_the_config_field(self):
        """Pins the call, so a revert to `cfg.agent.provider` reddens here.
