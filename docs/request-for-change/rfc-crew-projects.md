...
is credential-scrubbed; only allowlisted OS keychain helpers and the exact trusted
`gh auth git-credential` command may be restored from system/global Git config.
...
token if any of that content changed after the owner reviewed it. A managed bundle
sync compares the newly fetched digest with the activation record; a change
withdraws the tracked agents, skills, and MCP entries and leaves the Project
inactive until the owner reviews and activates the new content. Bundle-only prose
or display-name changes do not expand the trust grant.
...
the slot is created. Project-domain exception text is credential- and suspicious-
URL-redacted before it reaches dashboard JSON.
...
credentials, `allowedTools`, or MCP `autoApprove`.
