...
any change after review makes activation fail. The successful decision is critically
SEL-audited before any output is materialized.
...
instead of falling back to a name-based copy. Bundle-owned agent and MCP JSON
...
tree has fixed entry and byte budgets during the pinned copy. The stored digest
...
`allowedTools`; nested agent MCP and standalone Project MCP definitions cannot
...
activate it explicitly. An unchanged key may refresh the existing activation.
