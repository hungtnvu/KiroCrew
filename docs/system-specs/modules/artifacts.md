...
the same way -- a provider that raises, and a destination reporting `available() ==
False`, which skips the delete entirely and so loses the handle just as silently.

Deleting the artifact is the way out of a kept publication, and it notifies the
destination. `delete_for_artifact` runs from the delete handler AFTER the local delete has
been committed, using the publication the handler already read for its version capture.
The ordering is deliberate. Running it BEFORE the local delete was tried and reverted: it
puts a network round trip inside the window between reading the artifact and removing it,
which turns a concurrent save into silent loss, and a failure there can fail a delete
request that could never afterwards succeed. Running it after costs a crash window -- a
process death between the two steps strands the copy -- which is the smaller residue,
because the local delete is what the user asked for and it is never at risk. The function
never raises, and the whole body is inside its guard: provider resolution and the
availability probe can both fail on a publication naming a destination this edition does
not register, and a caller that has already committed the delete cannot act on an
exception. A destination failure therefore costs a log line naming the slug and provider,
and the residue is bounded and visible: a leftover object in the owner's own bucket.
