...
raises, so a leftover JWT cannot stay usable). Off and Login both
revoke those sessions and the proxy when `apply_agentcore_runtime`
returns False (policy reload failed); PUT is 503
`runtime_apply_failed` so a 200 cannot mean a non-workload posture
...
every restored name survives command validation; that same
...
survive); a login rebuild's fail-closed refresh snapshots those
user-owned fields, unlinks materialized agents so a removed or
malformed leftover cannot stay executable, aborts when any
``<app>--*.json`` remains after that unlink (a swallowed
``OSError`` must not leave the command executable), rematerializes with
the snapshot merged in, and aborts the rebuild on any I/O
failure so a leftover command cannot sit under a successful
login emit. Originals stay until that rematerialize
succeeds; stale `<app>--*.json` names are pruned only
afterward. Gateway boot runs the host rebuild off the
