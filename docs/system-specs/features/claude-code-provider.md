...
left in `ACP_BACKENDS_KNOWN` to add. `register_selectable_backend` remains for a
harness the core does **not** ship — an edition registers its own id from
`ProviderRegistry.register_acp_backends` and the id reaches the config gate, the
dashboard PATCH allowlist and `GET /api/config/schema` with no core edit
