...
def _read_project_json_object(path: Path, root: Path, *, label: str) -> dict[str, Any]:
    """Read bundle JSON while validating the same opened handle that supplies its bytes."""
    payload = _read_project_file_bytes(path, root, label=label)
    try:
...
def _capability_review_key(
    ...
    *,
    ...
    for path in bundle.agents:
        ...
        _digest_field(
            digest,
            _read_project_file_bytes(path, bundle.root, label=f"Project agent {path.name}"),
        )
    ...
    if bundle.manifest.context.mcp:
        mcp_path = bundle.root / bundle.manifest.context.mcp
        _digest_field(digest, b"mcp")
        _digest_field(digest, bundle.manifest.context.mcp.encode("utf-8"))
        _digest_field(
            digest,
            _read_project_file_bytes(mcp_path, bundle.root, label="Project MCP config"),
        )
    return f"{root_key}#{digest.hexdigest()}"


def _validate_agent(path: Path, root: Path) -> dict[str, Any]:
    spec = _read_project_json_object(path, root, label=f"Project agent {path.name}")
...
def _validate_mcp(path: Path, root: Path) -> dict[str, dict[str, Any]]:
    document = _read_project_json_object(path, root, label="Project MCP config")
...
class ProjectCapabilityManager:
    ...
    def _resolve(self, identifier: str, *, validate_executable: bool = False) -> _ResolvedBundle:
        ...
        if manifest.context.mcp:
            ...
            if validate_executable:
                mcp_servers = _validate_mcp(mcp_path, root)
            else:
                document = _read_project_json_object(mcp_path, root, label="Project MCP config")
                raw_servers = document.get("mcpServers", {})
                if not isinstance(raw_servers, dict):
                    raise ProjectCapabilityError("Project MCP config needs an mcpServers object")
                mcp_servers = {
        ...
        return _ResolvedBundle(project, root, manifest, agents, skills, mcp_servers)
    ...
    def _remove_materialized(self, state: dict[str, Any], *, strict: bool) -> None:
        if strict:
            self._verify_materialized(state)
        raw_agents = state.get("agents", {})
    ...
    def activate(
        ...
        *,
        ...
        with self._lock(project_id):
            ...
            validated_agents = [
                (path, _validate_agent(path, bundle.root)) for path in bundle.agents
            ]
            skill_digests = {
