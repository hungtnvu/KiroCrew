...
def _spawn_chat_folder_icon_task(
    ...
    *,
    ...
    one model call — per folder. A superseded task that has already begun its
    store write finishes that write under the lock before unwinding, so
    persisted snapshots stay strictly serialized.
    """
...
async def api_chat_folder_update(request: web.Request) -> web.Response:
    ...
    if regenerate_icon:
        ...
        # new icon when it lands; the epoch captured here (after the committed
        # mutation, so a rename in this same request has already bumped it via
        # the post-commit hook) pins the
        ...
        # name ``_apply`` captured under the store lock, so it derives from
        # exactly the committed state — a rename in this same request, and any
        # concurrent rename that committed first, are both reflected.
        _spawn_chat_folder_icon_task(
            state,
            fid,
