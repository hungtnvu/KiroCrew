...
async def api_agentcore_identity_save(request: web.Request) -> web.Response:
    ...
    applied = False
    rebuilt = False
    # Login and Off both revoke IAM Gateway access. A Workload→Login
    # apply failure must not leave the SigV4 proxy and old sessions up.
    ...
    async with _get_config_lock():
        ...
        if applied or revoking_workload:
            ...
            try:
                await _drop_live_identity(request, reset_proxy=revoking_workload)
            except Exception:
                logger.warning(
    ...
    if revoking_workload and not applied:
        await _audit_async(
            request,
            operation=OP_SAVE,
            outcome="error",
