    ...
    try:
        ...
        async for event in event_stream:
            ...
            if event.kind == EVENT_TEXT_CHUNK:
                ...
                # and defaults to "acp", so it is a constant naming no backend at
                # all. Reading it labelled every dashboard turn "acp" — a
                # claude_code turn and a KAS turn included — which makes a
                # provider split meaningless and disagrees with the label
                # subagent_manager already writes for the same backends.
                # provider_label speaks that same vocabulary ("acp" /
                # "claude_code" / "kas") and never raises: an object it does not
                # recognise degrades to "acp", exactly what the config said
