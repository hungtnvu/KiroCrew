...
def _is_managed_gateway_leftover(spec: Any) -> bool:
    ...
    Gateway hostname and the spec has no operator extras (headers,
    env, command). Other remotes under the reserved name stay.
    ...
    url = spec.get("url")
    if not (isinstance(url, str) and is_agentcore_gateway_url(url)):
        return False
    # Inject never persists headers or operator extras. Those fields are
    # an authored remote under the reserved name and must survive rebuild.
    return set(spec) <= {"url", "type", "timeout", "disabledTools", "autoApprove"}
...
def _refresh_enabled_app_agents(*, fail_closed: bool = False) -> None:
    ...
    ``fail_closed`` aborts the rebuild when any enabled app's refresh
    hits an I/O error, so a login emit cannot succeed while a leftover
    command is still on disk. Originals stay until replacements
    succeed; stale ``<app>--*.json`` names are pruned only after
    that write. A suppressed prune unlink is treated as a failure
    when any leftover name remains. User-owned fields (``model``,
    ``description``, extra ``toolsSettings``) are snapshotted and
    merged into the rematerialized files so a login rebuild cannot
    discard hand edits. Leave-login uses the default (in-place
    rematerialize, no prune) so ``_preserve_user_agent_edits`` can
    read the current files. Gateway boot skips this walk
    (``rebuild_agent_config(app_agent_refresh=False)``).
    """
    failures: list[str] = []
    try:
...
def _stash_authored_mcp(config: dict[str, Any], managed_names: set[str]) -> None:
    ...
    if not payload["mcpServers"] and not payload["tools"] and not payload["allowedTools"]:
        ...
        try:
            ...
            raw = {}
        # OSError / ValueError propagate: an unreadable existing stash
        # must not be replaced by a partial extract.
        if isinstance(raw, dict):
            loaded = raw
        if loaded:
...
def rebuild_agent_config(*, clean: bool = False, app_agent_refresh: bool = True) -> Path:
    ...
    if login_withhold:
        ...
        # Walks every enabled app. Identity PUT offloads the whole
        # rebuild via ``asyncio.to_thread``. Gateway boot passes
        # ``app_agent_refresh=False`` so this cannot delay readiness.
    ...
    if not login_withhold and app_agent_refresh:
        # Host spec is restored above. App-agent files still carry the
        # stripped mcpServers from the login rebuild until this refresh.
        # In-place (not fail_closed): rematerialize so
