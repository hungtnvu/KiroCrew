...
The refusal is about the shape, not the destination. Reading a local file into
an outbound request body is indistinguishable from exfiltration, so every
spelling is blocked: `-d @file`, `--data-binary @file`, `--data-urlencode @file`,
`-F field=@file`, `--upload-file`, `wget --post-file`, and `/dev/tcp/`
redirects.

Send the body inline instead — `curl -d '{"k":"v"}' https://example.com` — or use
a tool that takes the request body as an argument.
