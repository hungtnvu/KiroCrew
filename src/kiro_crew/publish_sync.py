...
async def unpublish(slug: str) -> None:
    ...
    retryable. Deleting the artifact is the blunt way out of a kept publication -- that
    path drops the record without notifying the destination, so it leaves the published
    copy where it is. Raises ``NotPublishedError`` only when the artifact isn't
    published.
    """
    store = get_default_store()
    art = await asyncio.to_thread(store.get, slug)
