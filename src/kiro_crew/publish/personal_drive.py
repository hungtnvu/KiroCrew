...
class PersonalDriveProvider(PublishProvider):
    ...
    # setup surface, where it is actionable.
    ...
    def _find_drive(self, profile: str, region: str) -> Optional[dict[str, str]]:
        ...
        for mapping in data.get("ResourceTagMappingList", []):
            ...
            if arn.startswith("arn:aws:s3:::"):
                ...
                distributions.append(arn.rsplit("/", 1)[-1])
                dist_arns.append(arn)
        if len(buckets) > 1 or len(distributions) > 1:
            raise PublishError(
                "This account has more than one personal drive, so it is not clear "
                "which one to use and nothing was touched. Buckets: "
                f"{', '.join(sorted(buckets)) or 'none'}; distributions: "
                f"{', '.join(sorted(distributions)) or 'none'}. Remove the extra one "
                "in the AWS console, then publish again."
            )
        if not buckets:
            return None
        return {
            "bucket": buckets[0],
            "distribution_id": distributions[0] if distributions else "",
            "distribution_arn": dist_arns[0] if dist_arns else "",
        }
    ...
    def _sandbox_headers_policy(self, profile: str) -> str:
        ...
        for item in items:
            ...
            if cfg.get("ResponseHeadersPolicyConfig", {}).get("Name") == name:
    ...
    def _serving_notice(self, dist_id: str, status: str, profile: str) -> str:
        ...
        if status and status != "Deployed":
            return (
                f"The drive's delivery network is still rolling out this change "
                f"(state: {status}). Published content becomes reachable when it "
                f"finishes -- usually a few minutes. Nothing was lost; try again "
                f"shortly and the same link will work."
            )
