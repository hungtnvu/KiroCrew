...
def _find_credential_store_dirs() -> list[str]:
    """The fence entries whose parent is the home directory, as real paths.

    These are the third-party credential stores (``.aws``, ``.ssh``, ``.gnupg``,
    ``.netrc``, ...): the fence covers everything inside them, so a name resolved
    inside one is fenced whatever the name is. Derived from the live list by the
    same single-segment / multi-segment split ``_SENSITIVE_LEAF_PARENT_DIRS``
    uses, rather than hand-listed, so an entry added later is covered with no
    ...
    """
    home = os.path.expanduser("~")
    return [os.path.join(home, entry) for entry in _SENSITIVE_HOME_DIRS if "/" not in entry]


#: Cap on the entries read from one credential store when matching a PATTERN
#: against its real contents. A literal name takes the exact ``os.path.exists``
#: route below and is never capped, so no literal is missed however large the
#: directory; this bound only limits how far a glob or regex is chased in a store
#: nobody expects to be huge (``.aws``, ``.ssh``).
_FIND_STORE_SCAN_LIMIT = 1024


def _find_store_holds_match(
    store: str,
    literal_names: list[str],
    name_matchers: "list[re.Pattern[str]]",
) -> str | None:
    """Does *store* actually hold a file this traversal's filters would match?

    Asking the STORE rather than reasoning about the pattern is what keeps the
    clause honest in both directions: ``-name credentials`` and ``-name 'id_*'``
    are the same question, and ``-name '*.py'`` answers no because no such file is
    in there. An earlier version tested only a LITERAL name, which denied the
    literal spelling of a read while allowing the glob spelling of the identical
    read (found in review).

    A ``-regex`` filter never reaches here: the caller reads it as opaque rather
    than running an agent-supplied regex on this thread, so the traversal is
    already denied by the wider reading before any store is scanned.

    Only the top-level credential stores are scanned, never a crew leaf-parent:
    the fence covers EVERY file inside a store, so any name found there is fenced,
    whereas ``~/.kiro/crew`` legitimately holds readable files (``config.json``,
    ``sessions.db``) beside its fenced leaves. Scanning that directory would deny a
    traversal for a file the fence does not cover.
    """
    for name in literal_names:
        probe = os.path.join(store, name)
        if os.path.exists(probe):
            return probe
    if not name_matchers:
        return None
    try:
        with os.scandir(store) as entries:
            for count, entry in enumerate(entries):
                if count >= _FIND_STORE_SCAN_LIMIT:
                    break
                if any(m.fullmatch(entry.name) for m in name_matchers):
                    return os.path.join(store, entry.name)
    except OSError:
        # Not a directory, or unreadable. A single-segment entry can be a FILE
        # (``.npmrc``), in which case there is nothing inside it to reach.
        return None
    return None
...
def _find_traversal_reaches_fence(
    ...
    unfiltered = opaque or not (name_pats or path_pats or regex_pats)
        ...
        for form in _candidate_forms(root):
            ...
            for store in stores:
                ...
                if not any(form.casefold().startswith(prefix) for form in _candidate_forms(store)):
                    continue
                held = _find_store_holds_match(store, literal_names, name_matchers)
