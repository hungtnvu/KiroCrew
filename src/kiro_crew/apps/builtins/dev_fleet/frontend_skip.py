...
def may_skip_frontend(git: str, repo: str, ref: str) -> bool:
    ...
    Returns ``True`` only on STRONG evidence of BOTH conditions:

      * the incoming sync changes nothing under ``website/``, AND
      * the on-disk ``node_modules`` verifiably satisfies the incoming lockfile.

    Everything else -- a website/ change, an unverifiable tree, or git being
    unavailable to answer either question -- returns ``False`` and the sync
    builds as it does today. Dist freshness rides on the same diff-empty
    condition: when this sync changes nothing under ``website/``, the bundle it
    would build is byte-for-byte the one already staged, so declining to rebuild
    serves the same bytes. A dist left stale for a reason UNRELATED to this sync
    is a pre-existing condition this sync neither created nor is obligated to
    repair -- and this sync would not have changed it either way, because it
    touches nothing under ``website/``.
    """
