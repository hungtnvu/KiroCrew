...
#: Every value ``outcome`` can carry. ``other`` covers a terminal-looking status
#: outside this set, for the same bounding reason as :data:`TOOL_KIND_OTHER`.
OUTCOMES = frozenset({OUTCOME_COMPLETED, OUTCOME_FAILED, OUTCOME_CANCELLED, TOOL_KIND_OTHER})
