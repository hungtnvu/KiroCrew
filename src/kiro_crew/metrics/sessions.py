...
#: watchdog recycle, compaction recycle). See the module docstring.
...
# Keys whose end has already been recorded. The crumb write is handed to a
# worker thread, so a session that starts and ends inside that handoff would
# otherwise have its writer land AFTER the end consumed nothing -- leaving a
# crumb no teardown will ever claim, which the next boot reads as a crash that
# never happened. Recording the end here and checking it in the writer makes the
# two ordered regardless of thread scheduling. A start clears its own key first,
# so a later genuine session under the same key still gets a crumb.
#
# Cleared wholesale past the cap rather than grown for the process lifetime: the
# cost is one missed suppression, never a leak.
_MAX_ENDED_KEYS = 4096
_ended_lock = threading.Lock()
_ended_keys: set[str] = set()


def _note_ended(session_key: str) -> None:
    with _ended_lock:
        if len(_ended_keys) >= _MAX_ENDED_KEYS:
            _ended_keys.clear()
        _ended_keys.add(session_key)


def _clear_ended(session_key: str) -> None:
    with _ended_lock:
        _ended_keys.discard(session_key)


def _has_ended(session_key: str) -> bool:
    with _ended_lock:
        return session_key in _ended_keys
...
def record_session_started(session_key: str) -> None:
    """Count one session start and drop its crumb; never raises.

    Idempotent per session key: an existing crumb is left exactly as written, so
    a key re-entering the registry while its first crumb is still open neither
    double-counts the start nor loses the original start time.

    Safe to call while the session registry lock is held. The counter is
    in-memory, and the crumb write is handed to a worker thread whenever an
    event loop is running -- every call site here sits inside
    ``async with owner._lock``, and that lock must not span blocking IO. The
    start time is stamped BEFORE the handoff, so the recorded value is when the
    session was admitted, not when the writer thread got scheduled.
    """
    if not session_key:
        return
    try:
    ...
    _clear_ended(session_key)
    try:
        loop = asyncio.get_running_loop()
    except RuntimeError:
        _write_crumb(session_key, started_at)
        return
    try:
        loop.run_in_executor(None, _write_crumb, session_key, started_at)
    except Exception:
...
def _write_crumb(session_key: str, started_at: float) -> None:
    ...
    Refuses when the session has already ended: this runs on a worker thread, so
    a short-lived session can reach teardown first, and writing then would leave
    a crumb nothing will consume for the next boot to misread as a crash.
    """
    if _has_ended(session_key):
        return
    try:
        ...
        # Re-checked after the write: the end could have landed while it was in
        # flight, and the crumb must not outlive the session it describes.
        if _has_ended(session_key):
            _unlink(path)
    except Exception:
...
def record_session_ended(session_key: str, *, end_reason: str) -> None:
    """Emit *session_key*'s lifetime and consume its crumb; never raises.

    No crumb means no emit. That is the whole double-count defence: the six
    teardown paths are not mutually exclusive (the idle sweep calls ``reset``),
    and whichever one reaches the session first is the one that records it.

    It also bounds what the deferred crumb write can cost: a session torn down
    before its writer thread ran has no crumb, so it contributes no sample. Only
    sessions that ended within roughly a thread handoff of starting can land
    there, and their sub-millisecond lifetimes would be skipped by the
    non-positive-duration rule regardless.
    """
    if not session_key or end_reason not in END_REASONS:
        return
    # Before the read, so a crumb writer still in flight on a worker thread sees
    # the end and declines to leave a crumb behind (see _write_crumb).
    _note_ended(session_key)
    try:
        path = _crumb_path(session_key)
        started_at = _read_started_at(path)
        # Unlink before the emit: a session must be consumed even when its crumb
        # is unreadable or its duration unusable, or a corrupt crumb is re-walked
        # by every boot from now on.
        _unlink(path)
...
def _read_started_at(path: Path) -> float | None:
    return _read_crumb(path)[1]
