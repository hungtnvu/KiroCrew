...
#: One per subagent admitted to run. ``concurrency`` is the number already
#: running when it was admitted — an integer bounded by the spawn cap, so the
#: series stays small and the aggregator's MAX over it IS the concurrency
#: high-water mark. A separate high-water instrument would need its own
#: reset semantics and could not be read per-attribute like this.
