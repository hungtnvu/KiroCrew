...
export class HeightCache {
   ...
   * live again. Retired entries keep their LRU position, so they are evicted
   * before live ones and cannot accumulate.
   *
  ...
  private evictToCap(): void {
    ...
    while (this.cache.size > cap) {
      ...
      // A retired entry is already out of measuredSum; dropping it from the map
      // must not subtract twice.
      if (!this.retired.delete(oldestKey)) this.measuredSum -= oldestVal
      evicted++
    }
    if (evicted > 0) {
      this.dirty = true
