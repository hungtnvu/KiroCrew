...
describe('useVirtualChat: height-sync spacer repricing keeps the top visible row anchored', () => {
  ...
  it('retires a swapped-out row even though the count never moved', () => {
    ...
    // The retirement is recorded in that commit, and lands in the offset tree at
    // the next sync -- an equal-count commit invalidates neither the offset memo
    // nor the anchor consumer (see TRIGGER 6's note and #7234), while a streaming
    // turn re-syncs on every measurement. One more real message is that sync.
    act(() => { view.result.current.measureRef(2)(nodeOf(300)) })
    act(() => { vi.advanceTimersByTime(200) })

    // The mean is the three real messages, so every unmeasured row -- the 26
    // plus the unmeasured replacement -- is priced at 300. With the ghost still
    // counted the mean would be 230 and the total ~7110.
