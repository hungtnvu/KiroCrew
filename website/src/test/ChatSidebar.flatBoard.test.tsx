...
describe('flat view inside the board', () => {
  ...
  it('hides the per-column New-folder button in flat view', () => {
    // Folders never render inside the columns in flat view, so a created
    // folder would appear to vanish (modal closes, board looks unchanged).
    const { container, getByTestId } = renderSidebar()
    expect(within(columnEl(container, 'col-all')).queryByTestId('column-new-folder-col-all')).toBeTruthy()
    fireEvent.click(getByTestId('flat-view-toggle'))
    expect(within(columnEl(container, 'col-all')).queryByTestId('column-new-folder-col-all')).toBeNull()
