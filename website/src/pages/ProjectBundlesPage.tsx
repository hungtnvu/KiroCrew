...
const TEXTAREA_CLASS = 'mt-2 w-full min-h-[88px] resize-y rounded-md border border-border bg-bg-elevated px-3 py-2 text-sm text-text outline-none transition-colors focus-ring'
        ...
        <form className="mt-5 space-y-6" onSubmit={event => { event.preventDefault(); if (canSave && !mutation.isPending) save() }}>
          <section className="space-y-3">
            ...
            <Field id="project-edit-description" label={i18nT('pages.projectBundlesPage.description')}>
              <textarea id="project-edit-description" name="project-description" aria-label={i18nT('pages.projectBundlesPage.description')} className={TEXTAREA_CLASS} value={description} onChange={event => setDescription(event.target.value)} />
            </Field>
