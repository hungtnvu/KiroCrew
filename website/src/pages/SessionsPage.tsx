...
/** Bucket a timestamp against local-midnight boundaries (DST-safe: day
 *  arithmetic goes through the Date constructor, never fixed 86400s). */
export function recencyGroup(tsMs: number, nowMs: number): RecencyGroup {
  ...
  if (tsMs >= startToday) return 'today'
  const startYesterday = new Date(n.getFullYear(), n.getMonth(), n.getDate() - 1).getTime()
  if (tsMs >= startYesterday) return 'yesterday'
  return 'earlier'
}

/** A slot's last-activity instant in ms; 0 when nothing parseable exists so
 *  undated slots sort last within "earlier" instead of throwing. */
export function slotTsMs(s: ChatSlot): number {
  const raw = s.last_activity_ts || s.last_ts || s.created
...
export default function SessionsPage() {
  ...
  const groups = useMemo(() => {
    ...
    for (const s of filtered) by[recencyGroup(slotTsMs(s), now)].push(s)
    for (const g of GROUP_ORDER) by[g].sort((a, b) => slotTsMs(b) - slotTsMs(a))
    return by
  }, [filtered])
